...
class ReaderServiceTest {
  ...
  void readWithError() {
    ...
    assertThrows(ResourceNotFoundException.class, () -> this.readerService.read("1", 22));
  }

  @Test
  void downloadWithError() {
    when(this.comicRepository.findById(any())).thenReturn(Optional.of(ComicFixtures.COMIC_V1_1.toBuilder()
        .path("/foo/bar.cbz")
        .build()));

    assertThrows(ResourceNotFoundException.class, () ->
        Objects.requireNonNull(this.readerService.download("1").getBody()).writeTo(null));
  }
